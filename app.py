...
st.markdown(
    "This app fetches police stations for **every Indian district** using the **Overpass API** (OpenStreetMap). No API key needed."
)
...
@st.cache_data
def load_districts():
    # Ensure your CSV is formatted with two columns: State,District
    return pd.read_csv("india_districts.csv", encoding="utf-8-sig")
...
if st.button("🚀 Fetch All India Police Stations"):
    ...
    for i, row in enumerate(districts_df.itertuples(), 1):
        ...
        for e in elems:
            ...
            addr = ", ".join(
                [tags.get(k, "") for k in ["addr:street", "addr:city", "addr:state"] if tags.get(k)]
            )
            lat, lon = (
                (e.get("lat"), e.get("lon")) if "lat" in e else (e.get("center", {}).get("lat"), e.get("center", {}).get("lon"))
            )
        ...
        time.sleep(1)  # avoid overloading Overpass API

    # Convert to JSON bytes
    json_bytes = io.BytesIO(json.dumps(result, ensure_ascii=False, indent=2).encode("utf-8"))

    st.download_button(
        "⬇️ Download All-India police_stations.json",
        data=json_bytes,
